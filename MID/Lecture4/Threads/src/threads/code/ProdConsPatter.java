...
/**
 * Producer-Consumer pattern is one of the most important pattern to write
 * parallel/concurrent programs. Producer-Consumer is the situation where one or
 * more threads are producing data items and adding them to a shared data store
 * of some kind while one or more other threads process those items, removing
 * them from the data store.
 */

public class ProdConsPatter {
	...
	private static void consumer() throws InterruptedException {
		...
		while (true) {
			...
			if (random.nextInt(10) == 0) {
				Integer value = queue.take();// if queue is empty wait
